...
// ———————————————— UPTIME ROUTE FOR RENDER ———————————————— //
const app = express();
const UPTIME_PORT = config.dashBoard?.port || config.serverUptime?.port || process.env.PORT || 3001;

app.get('/uptime', (req, res) => {
    res.json({ status: "ok", message: "Bot is alive 🚀" });
});

app.listen(UPTIME_PORT, () => {
    log.info("SERVER", `Uptime route ready at http://localhost:${UPTIME_PORT}/uptime`);
});
